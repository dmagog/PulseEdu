...
- [ ] Чтение настроек из `.env` и из админ‑настроек (в БД).
- [ ] **Системная переменная «фиктивная дата»**: `APP_NOW_MODE=real|fake`, `APP_FAKE_NOW=YYYY-MM-DD`.
- [ ] Хелпер `now()` для сервиса: читает fake/real и используется в дедлайнах/метриках.
- [ ] Админ‑страница: просмотр/правка `APP_NOW_MODE/APP_FAKE_NOW` (dev‑режим).
...
- [ ] Alembic: базовая миграция, таблицы `User`, `Role`, `UserRole`.
- [ ] `database/engine.py`, `session.py` (SessionLocal, get_session()).
- [ ] `/auth/verify`: принимает `login`, любой пароль; создаёт пользователя при первом входе; cookie‑сессия.
- [ ] **`user_auth_log`**: логируем каждую попытку (успех/фейл) с `ts`, login, ip?, user_agent?.
**Тест:** войти/ошибиться логином → записи появляются в `user_auth_log`.

### I3 — Импорт Excel (схематично)
- [ ] `services/import_service.py`: парсинг по alias из `EXCEL_MAPPING_FILE`.
- [ ] `ImportJob` + протокол ошибок в `errors_json`.
- [ ] `/import` (route): загрузка файла (роль «Оператор данных»), постановка задачи в `ingest`.
- [ ] Worker `ingest`: валидация и сохранение сущностей.
**Тест:** загрузить демо‑Excel → `ImportJob.status=success`.

### I4 — **Поток студента (UI‑база)**
- [ ] Страница «Студент»: карточки курсов, **лента событий**, виджет дедлайнов (визуально, без сложной логики).
- [ ] Навигация к курсам/оценкам; минимальные заглушки данных.
- [ ] Ограничение доступа: студент видит только свои данные.
**Тест:** открыть `/student` → видна лента и прогресс (на заглушках/простых агрегациях).

### I5 — **Роли: Администратор и Оператор данных**
- [ ] **RBAC**: проверка доступа на уровне роутеров; редирект при недостаточных правах.
- [ ] `/admin`: базовые настройки (LLM, мониторинг, ретеншн, «тестовые часы»), просмотр системных метрик.
- [ ] `/import`: UI для загрузки Excel и просмотр журнала `ImportJob` (роль «Оператор данных»).
...
- [ ] `metrics_service.py`: вычисление статусов (submitted/on_review/late/missing) и прогресса.
- [ ] Учёт «тестовых часов» (`now()`), пересчёт после импорта.
- [ ] Celery Beat: периодический пересчёт дедлайнов (dev‑частота низкая).
...
- [ ] Сводка по курсам/группам: список студентов «в зоне внимания».
- [ ] Просмотр ленты событий по группе/курсу; фильтры.
- [ ] (Placeholder) блок рекомендаций — пока пустой.
**Тест:** открыть `/teacher` → видны агрегаты и списки рисков.

### I8 — Дашборды РОП
- [ ] Свод по программам/группам: проценты просрочек/прогресса.
- [ ] Мини‑графики на Chart.js (через CDN): тренды 7/30 дней.
- [ ] Навигация к деталям курса/группы.
**Тест:** открыть `/rop` → видны агрегаты и графики.

### I9 — **Кластеризация**
- [ ] `cluster_service.py`: простая кластеризация студентов по курсу (KISS, 2–3 признака).
- [ ] Worker/очередь `cluster`: запуск **после успешного импорта**.
- [ ] UI‑индикатор кластера в карточке курса (бейдж A/B/C …).
**Тест:** импорт → автозадача `cluster` отрабатывает, бейджи появляются.

### I10 — Многоворкерная схема
- [ ] Разнести задачи по контейнерам/очередям: `auth`, `ingest`, `email`, `llm`, `cluster` (+ `beat`).
- [ ] docker‑compose: отдельный `worker_*` на каждую очередь; `--concurrency`=1–2.
- [ ] Логи задач: `queue`, `task_name`, `job_id`.
**Тест:** импорт/кластеризация идут параллельно, логи с правильными метками.

### I11 — Уведомления (email)
- [ ] `email_service.py` + шаблоны Jinja для писем.
- [ ] Триггеры: завершение импорта/пересчёта, приближение дедлайна.
- [ ] Dev: MailHog принимает письма.
