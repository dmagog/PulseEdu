...
from app.models.llm_models import LLMRecommendation, LLMFeedback
...
async def get_recommendations_result(
    ...
    try:
        ...
        cached_rec = db.query(LLMRecommendation).filter(
            LLMRecommendation.student_id == student_id,
            LLMRecommendation.course_id == course_id
        ).order_by(LLMRecommendation.created_at.desc()).first()
        
        if not cached_rec:
            return {
                "status": "not_found",
                "message": "No recommendations found. Please generate them first."
            }
        ...
        if cached_rec.expires_at < datetime.utcnow():
            return {
                "status": "expired",
                "message": "Recommendations have expired. Please regenerate them."
            }
...
async def get_llm_stats(db: Session = Depends(get_session)):
    ...
    try:
        ...
        active_recommendations = db.query(LLMRecommendation).filter(
        ...
        student_ratings = db.query(LLMFeedback).filter(
            LLMFeedback.feedback_type == "student_rating"
        ).count()
        
        # Count teacher approvals
        teacher_approvals = db.query(LLMFeedback).filter(
            LLMFeedback.feedback_type == "teacher_approval"
        ).count()
        
        # Average rating
        avg_rating = db.query(func.avg(LLMFeedback.rating)).filter(
            LLMFeedback.feedback_type == "student_rating",
            LLMFeedback.rating.isnot(None)
        ).scalar() or 0
...
async def get_teacher_recommendations(
    ...
    try:
        ...
        for rec in recommendations:
            ...
            student_feedback = db.query(LLMFeedback).filter(
                LLMFeedback.recommendation_id == rec.id,
                LLMFeedback.feedback_type == "student_rating"
            ).first()
            
            # Get teacher feedback
            teacher_feedback = db.query(LLMFeedback).filter(
                LLMFeedback.recommendation_id == rec.id,
                LLMFeedback.feedback_type == "teacher_approval"
            ).first()
