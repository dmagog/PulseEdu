...
async def student_dashboard(
    request: Request, student_id: str = Query(default="01", description="Student ID"), db: Session = Depends(get_session)
) -> HTMLResponse:
    """
...
async def student_courses(
    ...
    try:
        ...
        return templates.TemplateResponse(
            ...
            {"request": request, "title": f"Курсы - {student_id}", "student": student, "courses": course_data},
        )
...
async def get_student_progress_api(student_id: str, db: Session = Depends(get_session)) -> Dict[str, Any]:
    """
    ...
    try:
        progress_data = student_service.get_student_progress(student_id, db)
        return {"status": "success", "student_id": student_id, "data": progress_data}
