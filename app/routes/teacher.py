...
from app.models.student import Attendance, Course, Lesson, Student, Task, TaskCompletion
...
async def teacher_dashboard(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
    ...
    try:
        ...
        return templates.TemplateResponse(
            "teacher/dashboard.html", {"request": request, "title": "Панель преподавателя", "dashboard": dashboard_data}
        )
...
async def course_details(course_id: int, request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
    ...
    try:
        ...
        try:
            ...
            for cluster_label in cluster_groups:
                cluster_groups[cluster_label].sort(key=lambda x: (-x["overall_progress"], x["student_id"]))
        except Exception as e:
        ...
        try:
            ...
            for group_id in students_by_group:
                students_by_group[group_id].sort(key=lambda x: (x["student"].name or x["student"].id).lower())
...
async def get_teacher_dashboard_api(db: Session = Depends(get_session)) -> Dict[str, Any]:
    """
...
async def get_course_details_api(course_id: int, db: Session = Depends(get_session)) -> Dict[str, Any]:
    """
...
async def get_course_clusters_api(course_id: int, db: Session = Depends(get_session)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "status": "success",
            "data": {"course_id": course_id, "clusters": cluster_groups, "total_clusters": len(clusters)},
        }
...
async def teacher_recommendations(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
    ...
    try:
        return templates.TemplateResponse(
            "teacher/recommendations.html", {"request": request, "title": "Управление рекомендациями"}
        )
...
async def teacher_courses(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
    ...
    try:
        ...
        return templates.TemplateResponse(
            "teacher/courses.html", {"request": request, "title": "Мои курсы", "courses": courses}
        )
...
async def teacher_students(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
    ...
    try:
        ...
        return templates.TemplateResponse(
            "teacher/students.html", {"request": request, "title": "Мои студенты", "students": students, "courses": courses}
        )
...
async def teacher_analytics(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
...
async def teacher_assignments(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
...
async def teacher_schedule(request: Request, db: Session = Depends(get_session)) -> HTMLResponse:
    """
