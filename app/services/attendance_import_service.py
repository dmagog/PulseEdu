...
from app.models.student import Attendance, Course, Lesson, Student
...
class AttendanceImportService:
                ...
                try:
                    ...
                    for lesson_col in lesson_cols:
                        ...
                        if lesson:
                            ...
                            existing = (
                                db.query(Attendance)
                                .filter(Attendance.student_id == student.id, Attendance.lesson_id == lesson.id)
                                .first()
                            )

                            if not existing:
                                attendance = Attendance(
                                    student_id=student.id, course_id=course.id, lesson_id=lesson.id, attended=attended
                                )
                                db.add(attendance)
    ...
    def _create_lessons(self, course_id: int, lesson_cols: List[str], db: Session) -> Dict[int, Lesson]:
        ...
        for lesson_col in lesson_cols:
            ...
            existing_lesson = (
                db.query(Lesson).filter(Lesson.course_id == course_id, Lesson.lesson_number == lesson_number).first()
            )

            if existing_lesson:
                lessons[lesson_number] = existing_lesson
            else:
                lesson = Lesson(course_id=course_id, lesson_number=lesson_number, title=lesson_col)
                db.add(lesson)
                db.commit()
                db.refresh(lesson)
