...
from app.models.ml_metrics import ClusteringQualityMetrics, MLModelPerformance, ClusteringAlert
...
class MLMonitoringService:
    ...
    def record_clustering_metrics(
        ...
        try:
            ...
            self._update_algorithm_performance(algorithm_used, algorithm_params, quality_metrics, processing_time, memory_usage, db)
            
            # Check for quality alerts
            self._check_quality_alerts(course_id, quality_metrics, algorithm_used, db, import_job_id)
            
            self.logger.info(f"Recorded clustering metrics for course {course_id}: algorithm={algorithm_used}, quality={quality_metrics.get('silhouette_score', 0):.3f}")
            return True
    ...
    def get_course_quality_history(self, course_id: int, days: int = 30, db: Session = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            metrics = db.query(ClusteringQualityMetrics).filter(
                and_(
                    ClusteringQualityMetrics.course_id == course_id,
                    ClusteringQualityMetrics.created_at >= cutoff_date
                )
            ).order_by(desc(ClusteringQualityMetrics.created_at)).all()
    ...
    def get_algorithm_performance_summary(self, days: int = 30, db: Session = None) -> Dict[str, Any]:
        ...
        try:
            ...
            performances = db.query(MLModelPerformance).filter(
                MLModelPerformance.updated_at >= cutoff_date
            ).all()
            
            # Get recent quality metrics
            recent_metrics = db.query(ClusteringQualityMetrics).filter(
                ClusteringQualityMetrics.created_at >= cutoff_date
            ).all()
    ...
    def _update_algorithm_performance(
        ...
        try:
            ...
            perf = db.query(MLModelPerformance).filter(
                and_(
            ...
            if perf:
                ...
                # Update averages
                perf.avg_silhouette_score = (perf.avg_silhouette_score * perf.total_runs + quality_metrics.get("silhouette_score", 0)) / total_runs
                perf.avg_calinski_harabasz_score = (perf.avg_calinski_harabasz_score * perf.total_runs + quality_metrics.get("calinski_harabasz_score", 0)) / total_runs
                perf.avg_combined_score = (perf.avg_combined_score * perf.total_runs + quality_metrics.get("combined_score", 0)) / total_runs
                ...
                perf = MLModelPerformance(
                    ...
                    threshold_met_count=1 if quality_metrics.get("silhouette_score", 0) >= self.quality_thresholds["silhouette_min"] else 0,
