...
class MetricsService:
    ...
    def get_upcoming_deadlines(self, days_ahead: int = 7, db: Session = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            upcoming = db.query(TaskCompletion).join(Task).filter(
                and_(
    ...
    def _calculate_overall_progress(self, attendance_metrics: Dict[str, Any], task_metrics: Dict[str, Any]) -> float:
        ...
        try:
            ...
            overall = (
                attendance_metrics.get("percentage", 0) * attendance_weight +
                task_metrics.get("percentage", 0) * task_weight
            )
    ...
    def get_system_metrics(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            active_students = db.query(Student).join(TaskCompletion).filter(
                TaskCompletion.created_at >= week_ago
            ).distinct().count()
