...
class LLMProvider:
    ...
    def _make_request(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        payload = {
            ...
            "completionOptions": {"stream": False, "temperature": self.temperature, "maxTokens": self.max_tokens},
            "messages": [
                {
                    "role": "system",
                ...
                if alternatives and "message" in alternatives[0]:
                    ...
                    for line in lines:
                        ...
                        if line and (
                            ...
                            or any(keyword in line.lower() for keyword in ["рекоменд", "совет", "предлаг"])
                        ):
                            # Clean up the line
                            clean_line = line
                            for prefix in ["1.", "2.", "3.", "-", "•", "*"]:
    ...
    def get_cached_recommendations(self, student_id: str, course_id: str, data_version: str) -> Optional[List[str]]:
        ...
        try:
            ...
            cached = (
                ...
                .filter(LLMRecommendation.cache_key == cache_key, LLMRecommendation.expires_at > datetime.utcnow())
                .first()
            )
    ...
    def cache_recommendations(
        self, student_id: str, course_id: str, data_version: str, recommendations: List[str], expires_hours: int = 24
    ) -> bool:
        ...
        try:
            ...
            db.query(LLMRecommendation).filter(LLMRecommendation.cache_key == cache_key).delete()
