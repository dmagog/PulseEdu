...
class MLClusterService:
    ...
    def __init__(self):
        ...
        self.params = {"n_clusters": 3, "random_state": 42, "n_init": 10, "max_iter": 300}
    ...
    def cluster_students_by_course(self, course_id: int, db: Session, import_job_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for student in students:
                ...
                if features:
                    student_features.append({"student_id": student.id, "features": features})
            ...
            clustering_results = {"total_students": len(students), "clustered_students": len(saved_clusters)}
            ...
            self.logger.info(
                f"ML clustering completed for course {course_id}: {len(saved_clusters)} students clustered using {best_algorithm}"
            )
            return result
    ...
    def get_student_cluster(self, student_id: str, course_id: int, db: Session) -> Optional[StudentCluster]:
        ...
        try:
            cluster = (
                ...
                .filter(and_(StudentCluster.student_id == student_id, StudentCluster.course_id == course_id))
                .first()
            )
    ...
    def get_course_clusters(self, course_id: int, db: Session) -> List[StudentCluster]:
        ...
        """
        try:
            clusters = db.query(StudentCluster).filter(StudentCluster.course_id == course_id).all()
    ...
    def _optimized_ml_clustering(
        self, student_features: List[Dict[str, Any]]
            ...
            try:
                ...
                if len(set(cluster_labels)) > 1:
                    ...
                    if silhouette >= self.quality_threshold:
                        ...
                        self.logger.warning(
                            f"Optimal algorithm quality {silhouette:.3f} below threshold {self.quality_threshold}"
                        )
                else:
    ...
    def _fallback_ml_clustering(
        self, student_features: List[Dict[str, Any]], feature_matrix_scaled: np.ndarray
    ) -> Tuple[Dict[str, List[Dict[str, Any]]], str, Dict[str, float]]:
        """
        ...
        try:
            ...
            fallback_algorithms = [
                {"name": "KMeans_Simple", "model": KMeans, "params": {"n_clusters": 3, "random_state": 42, "n_init": 5}},
                {
                    "name": "Agglomerative_Simple",
                    "model": AgglomerativeClustering,
                    "params": {"n_clusters": 3, "linkage": "ward"},
                },
                {"name": "DBSCAN_Simple", "model": DBSCAN, "params": {"eps": 0.5, "min_samples": 2}},
            ]
            ...
            for algo_config in fallback_algorithms:
                try:
                    ...
                    if len(unique_labels) < 2 or (
                        algo_config["name"].startswith("DBSCAN") and -1 in unique_labels and len(unique_labels) == 2
                    ):
                        continue
            ...
            self.logger.info(
                f"Fallback algorithm {best_algorithm} achieved quality {best_quality_metrics.get('silhouette_score', 0):.3f}"
            )
    ...
    def _ml_clustering(
        ...
        try:
            ...
            if len(unique_labels) > 1:
                ...
                clusters = self._convert_to_cluster_format(student_features, cluster_labels, feature_matrix, feature_names)
    ...
    def _convert_to_cluster_format(
        self,
        student_features: List[Dict[str, Any]],
            ...
            for i, (label, performance) in enumerate(sorted_clusters):
                ...
                for student_data in cluster_groups[label]:
                    ...
                    confidence = min(
                        ...
                        max(
                            0.0,
                            (features["attendance_rate"] + features["completion_rate"] + features["overall_progress"]) / 300,
                        ),
                    )
    ...
    def _simple_clustering_fallback(
        ...
        try:
            ...
            if course_id and db:
                saved_clusters = self._save_cluster_assignments(course_id, clusters, db, import_job_id, "Simple_Fallback", {})
            ...
            for cluster in clusters:
                if cluster.ml_metadata:
                    ...
                    if algorithm not in cluster_analysis:
                        cluster_analysis[algorithm] = {"count": 0, "quality_metrics": quality_metrics}
    ...
    def update_optimal_parameters(
        self, algorithm: str = "KMeans", params: Dict[str, Any] = None, quality_threshold: float = 0.3
    ) -> bool:
        """
