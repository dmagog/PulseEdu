...
from app.models.student import Course, Student, Task, TaskCompletion
...
class LearningImportService:
                    ...
                    for task_col in task_cols:
                        ...
                        if task:
                            ...
                            existing = (
                                ...
                                .filter(TaskCompletion.student_id == student.id, TaskCompletion.task_id == task.id)
                                .first()
                            )
    ...
    def _get_task_columns(self, columns: List[str]) -> List[str]:
        ...
        for col in columns:
            if (
                not col.startswith("Время выполнения")
                and not col.startswith("Дедлайн")
                and col not in ["Студент_ID", "Курс", "Дорожная карта курса"]
            ):
    ...
    def _create_tasks(self, course_id: int, task_cols: List[str], df: pd.DataFrame, db: Session) -> Dict[str, Task]:
        ...
        for task_col in task_cols:
            ...
            existing_task = db.query(Task).filter(Task.course_id == course_id, Task.name == task_col).first()
            ...
            if existing_task:
                ...
                task = Task(course_id=course_id, name=task_col, task_type=task_type, deadline=deadline)
                db.add(task)
                db.commit()
                db.refresh(task)
                tasks[task_col] = task
    ...
    def _log_error(
        self, job_id: str, row_number: int, column_name: str, error_type: str, error_message: str, cell_value: str, db: Session
    ):
