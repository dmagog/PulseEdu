...
class ClusterService:
    ...
    def cluster_students_by_course(self, course_id: int, db: Session, import_job_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" not in result:
                self.logger.info(
                    f"ML clustering completed for course {course_id}: {result.get('clustered_students', 0)} students clustered using {result.get('algorithm_used', 'Unknown')}"
                )
            else:
    ...
    def cluster_all_courses(self, db: Session, import_job_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" not in result:
                self.logger.info(
                    f"ML clustering completed for all courses: {result.get('total_clustered', 0)} students clustered across {result.get('successful_courses', 0)} courses"
                )
            else:
    ...
    def _save_cluster_assignments(
        self, course_id: int, clusters: Dict[str, List[Dict[str, Any]]], db: Session, import_job_id: Optional[str] = None
    ) -> List[StudentCluster]:
