...
# Configure logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s level=%(levelname)s logger=%(name)s msg=%(message)s")
...
# Create FastAPI app
app = FastAPI(title="PulseEdu", description="Educational analytics and recommendation system", version="0.1.1")
...
async def add_request_id(request: Request, call_next):
    ...
    logger.info(f"Request completed status_code={response.status_code} request_id={request_id}")
