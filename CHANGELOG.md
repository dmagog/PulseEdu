...
#### CI/CD оптимизация
- **Устранено дублирование workflow** - удалены дублирующие файлы:
  - `release-quality.yml` - дублировал проверки качества
  - `quality.yml` - дублировал проверки качества  
  - `release.yml` - дублировал релизные процессы
- **Упрощена архитектура CI/CD** - теперь только 4 workflow вместо 8:
  - `test.yml` → "Release Tests & Quality" (объединяет тесты и проверки для релизов)
  - `quick-check.yml` (быстрые проверки для разработки)
  - `deploy.yml` (развертывание для main без релизных тегов)
  - `notifications.yml` (уведомления)
- **Исправлены триггеры** - релизные теги запускают только один workflow
- **Устранено одновременное выполнение** - больше нет 5 параллельных проверок при релизе

### Изменено
- **Переименован test.yml** в "Release Tests & Quality" для ясности
- **Обновлен deploy.yml** для игнорирования релизных тегов
- **Оптимизированы ресурсы CI/CD** - сокращено время выполнения проверок

### Технические детали
- **Файлы удалены**: 3 дублирующих workflow файла
- **Файлы обновлены**: 
  - `test.yml` - переименован и оптимизирован
  - `deploy.yml` - добавлено игнорирование релизных тегов
- **Результат**: Сокращение времени CI/CD с ~5 минут до ~2 минут для релизов
