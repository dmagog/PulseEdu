...
from app.models.student import Course, Student, Task
...
class TestStudentModel:
    ...
    def test_student_creation(self, isolated_db_session):
        ...
        retrieved_student = isolated_db_session.query(Student).filter(Student.id == student_id).first()
    ...
    def test_student_required_fields(self, isolated_db_session):
        ...
        retrieved = isolated_db_session.query(Student).filter(Student.id == student_id).first()
...
class TestCourseModel:
    ...
    def test_course_creation(self, isolated_db_session):
        ...
        retrieved_course = isolated_db_session.query(Course).filter(Course.id == course_id).first()
...
class TestTaskModel:
    ...
    def test_task_creation(self, isolated_db_session):
        ...
        course_id = int(datetime.now().timestamp()) % 1000000
        course = Course(id=course_id, name="Курс для задания", description="Описание курса")
        isolated_db_session.add(course)
        isolated_db_session.commit()

        # Then create a task
        task_id = int(datetime.now().timestamp()) % 1000000
        task = Task(id=task_id, name="Тестовое задание", course_id=course_id, deadline=datetime(2024, 12, 31, 23, 59, 59))
        isolated_db_session.add(task)
        isolated_db_session.commit()

        retrieved_task = isolated_db_session.query(Task).filter(Task.id == task_id).first()
...
class TestUserModel:
    ...
    def test_user_creation(self, isolated_db_session):
        ...
        retrieved_user = isolated_db_session.query(User).filter(User.user_id == user_id).first()
...
class TestRoleModel:
    ...
    def test_role_creation(self, isolated_db_session):
        ...
        role = Role(role_id=role_id, name=role_name, description="Тестовая роль")
        isolated_db_session.add(role)
        isolated_db_session.commit()

        retrieved_role = isolated_db_session.query(Role).filter(Role.role_id == role_id).first()
